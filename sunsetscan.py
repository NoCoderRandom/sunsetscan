#!/usr/bin/env python3
"""
SunsetScan - Network EOL Scanner & Security Assessment Tool.

Main entry point for the SunsetScan CLI application. Provides network discovery,
banner grabbing, and End-of-Life (EOL) status checking for discovered services.

Usage:
    ./sunsetscan.py                    Launch interactive menu
    ./sunsetscan.py --target <CIDR>    Scan target directly
    ./sunsetscan.py --version          Show version and exit
    ./sunsetscan.py --help             Show help message

Author: SunsetScan Team
License: MIT
Python: 3.9+
"""
...
def check_scan_readiness() -> List[str]:
    ...
    if not has_eol_cache:
        warnings.append("EOL database empty — run: python3 sunsetscan.py --setup")

    # 3. CVE cache populated
    cve_path = cache_dir / "cve_cache.json"
    if not cve_path.exists() or cve_path.stat().st_size <= 100:
        warnings.append("CVE database empty — run: python3 sunsetscan.py --setup")
    ...
    try:
        ...
        if missing:
            warnings.append(
                ...
                f"({', '.join(missing)}) — run: python3 sunsetscan.py --setup"
            )
    except Exception:
        warnings.append("Default modules not installed — run: python3 sunsetscan.py --setup")
...
def _print_readiness_warnings(warnings: List[str]) -> None:
    ...
    try:
        ...
        console.print()
        console.print("  Run [bold]'python3 sunsetscan.py --setup'[/bold] to fix all issues.")
        console.print("=" * 60)
        console.print()
    except Exception:
        ...
        for w in warnings:
            print(f"  [!] {w}")
        print()
        print("  Run 'python3 sunsetscan.py --setup' to fix all issues.")
        print("=" * 60)
        print()
...
class SunsetScan:
    ...
    def __init__(self, args: argparse.Namespace):
        """Initialize SunsetScan application.
    ...
    def _hardware_eol_to_finding(
        ...
        """Convert a hardware lifecycle database match to a SunsetScan finding."""
        label = " ".join(
            part for part in [
        ...
        source = match.source_url or "SunsetScan hardware EOL database"
        ...
        if match.status == "lifecycle_review" or match.review_required:
            ...
            reason = match.reason or (
                "Vendor lifecycle data needs review before SunsetScan can confirm "
            ...
            return Finding(
                ...
                explanation=(
                    ...
                    "and support ended differently. SunsetScan keeps this as a "
        ...
        if match.mixed:
            ...
            return Finding(
                ...
                explanation=(
                    ...
                    "listed as unsupported, but SunsetScan could not confirm this "
    ...
    def run_security_checks(self, scan_result: ScanResult) -> FindingRegistry:
        ...
        if not self.cache.is_eol_cache_current():
            ...
            if eol_age is None:
                self.console.print("[yellow]WARNING: EOL data has never been downloaded. Run: python sunsetscan.py --setup[/yellow]")
            else:
                self.console.print(f"[yellow]WARNING: EOL data is {eol_age} days old. Run: python sunsetscan.py --update-cache[/yellow]")
    ...
    def export_report(self) -> None:
        ...
        default_name = f"sunsetscan_scan_{timestamp}.{format_type}"
        ...
        try:
            ...
            if port_scan_timed_out:
                self.finding_registry.add(Finding(
                    ...
                    explanation=(
                        "SunsetScan preserved the discovery results and continued "
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog='sunsetscan',
        description='SunsetScan - Network EOL Scanner & Security Assessment Tool',
...
examples:
  ...
  %(prog)s --update-cache                           Refresh CVE/EOL data
    ...
    parser.add_argument(
        '--check-version',
        action='store_true',
        help='Check for a newer SunsetScan release on GitHub and exit'
    )

    parser.add_argument(
        '--update',
        action='store_true',
        help='Pull latest SunsetScan code from GitHub and reinstall requirements'
    )
...
def run_setup_wizard(db_size: str = "normal") -> int:
    ...
    console.print("  Run: [bold]python3 sunsetscan.py --modules[/bold] to see all options")
    console.print("  Run: [bold]python3 sunsetscan.py --download all[/bold] for full coverage")
    ...
    console.print("\nSunsetScan is ready. Run:")
    console.print("  [bold]python sunsetscan.py -i[/bold]           (interactive mode)")
    console.print("  [bold]python sunsetscan.py --target 192.168.1.0/24[/bold]  (direct scan)")
    return 0
...
def run_update_cache() -> int:
    ...
    console = Console()
    console.print("\n[bold blue]SunsetScan Cache Update[/bold blue]")
    console.print("=" * 50)
...
def show_cache_status() -> int:
    ...
    table = Table(title="SunsetScan Cache Status", show_header=True)
...
def main() -> int:
    ...
    if args.identify:
        if not args.target:
            ...
            print("Example: python sunsetscan.py --identify --target 192.168.1.0/24")
            return 1
        app = SunsetScan(args)
    ...
    if args.full_assessment:
        if not args.target:
            ...
            print("Example: python sunsetscan.py --full-assessment --target 192.168.1.0/24")
            return 1
